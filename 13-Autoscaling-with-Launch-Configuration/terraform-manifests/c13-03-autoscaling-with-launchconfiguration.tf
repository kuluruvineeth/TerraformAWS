...
module "autoscaling" {
  ...
  instance_refresh = {
    ...
    triggers = ["tag","desired_capacity"] # Desired Capacity here added for demonstrating the Instance Refresh scenario
  ...
  metadata_options = {
    ...
    http_tokens = "optional" # At productio grade you can change to "required", for our example if it is optional we can get the content in metadata.html
